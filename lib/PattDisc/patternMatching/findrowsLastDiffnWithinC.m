function loc = findrowsLastDiffnWithinC(query_row, mtx, c)
...
% 1 - c of the last column.

% INPUT
%  query_row is an m-element vector.
%  mtx is an m x n matrix.
%  c is a constant in (0, 1]. Part of a token is the time difference
%   t2 - t1. When lookup is performed, some inexactness in this ratio is
%   permitted, as set by timeTolerance. A value of .75 permits a difference
%   of .25 either way, for example.
...
loc = find(sum([sum(repmat(query_row(1:mn(2)-1), mn(1), 1) ==...
  ...
  abs(mtx(:, mn(2)) - repmat(query_row(mn(2)), mn(1), 1)) <= 1 - c], 2)...
  == 2);
