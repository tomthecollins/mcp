...
%   similarFunc = 'normalised matching score', then similarParam takes
%   either a string value (one of 'normal', 'pitchindependent',
%   'tempoindependent', or 'tempoandpitchindependent') or it is a struct
%   specifying more parameter values (see fpgethistogram2 for details).
...
elseif ~isempty(P) && ~isempty(Q) % Situation (2).
  ...
  if mP > 1
    for iP = 1:mP
    % parfor iP = 1:mP
    % Did not seem to make use of the Parallel Computing Toolbox, so
    % commented it out.
      tP = P{iP};
      for iQ = 1:mQ
        if printProgressString
          fprintf('Row %d of %d, column %d of %d.\n', iP, mP, iQ, mQ)
        end
        tQ = Q{iQ};
        if strcmp(similarFunc, 'cardinality score')
          if similarParam
...
elseif isstruct(P) && isempty(Q) % Situation (3).
  ...
  for iP = 1:mP
    ...
    for jP = iP + 1:mP
      ...
      elseif strcmp(similarFunc, 'normalised matching score')
        ...
        fphist = h_nsm(tP, tQ, similarParam);
        if ~isempty(maxscore) &&... % Empty -> fault with fpgethistogram. 
            ~isempty(fphist) % Empty histogram is returned for no matches.
  ...
  % Had to include this extra loop because the commented-out line above
  % caused issues when parallelising the code.
  for iP = 1:mP
    for jP = iP + 1:mP
      s(jP, iP) = s(iP, jP);
    end
