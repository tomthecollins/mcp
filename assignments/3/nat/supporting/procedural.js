...
Tone.Transport.schedule(function(time){
	...
	noteOns.map(function(a){
		synth.triggerAttack(
			a[1],
			a[0] + Tone.Transport.toSeconds(Tone.Transport.Position))
	});

	noteOffs.map(function(b){
		synth.triggerRelease(
			b[1],
			b[0] + Tone.Transport.toSeconds(Tone.Transport.Position));
	});
 
}, 14*60/Tone.Transport.bpm.value)

var metronome2 = new Tone.NoiseSynth().toMaster();
var xcrptBeats = [0.001, 1, 2, 3, 4, 5, 6, 7];
